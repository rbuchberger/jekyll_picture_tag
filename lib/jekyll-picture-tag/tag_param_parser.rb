module PictureTag
  def parse_tag_params(raw_params, context)
    ...
    instructions[:source_images].default = params.shift
    ...
    while params.first =~ /(?<source_key>\w+):/
      ...
      instructions[:source_images][source_key] = params.shift
    end

    # Anything left will be html attributes
    instructions[:html_attributes] = params.join ' '
    instructions
  end
